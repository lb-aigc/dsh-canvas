...
declare module '@deepseek-ai/dsh-session/types' {
  interface SessionEventMap {
    ...
    'canvas/state': { state: CanvasState }
  }
}

// ---- Session projection: the client read face (`useProjection('canvas')`). ----
...
function sessionOf(exec: unknown): Session | undefined {
  return (exec as { agent?: { session?: Session } }).agent?.session
}

function defineCanvasTools() {
  const requireSession = (exec: unknown): Session => {
    const session = sessionOf(exec)
    if (session === undefined) throw new Error('canvas 工具需要当前会话（无法确定会话身份）')
    return session
  }
  ...
  const addNodeTool = defineTool({
    ...
    description:
      '在画布上新增一个节点。用于把素材（图片/视频/音乐）或文字/笔记组织到画布上，供后续参考或连线。返回新节点的 id。',
    parameters: {
    ...
    if (event.type === 'user/message') {
      ...
      queueMicrotask(() => {
        try {
          ...
          //    on a single generated image (many-to-one), not one placeholder per
          //    image. The generated image later fills this single placeholder.
          const sources: CanvasNode[] = []
          for (const block of blocks) {
            const source = next.nodes.find((node) => node.url === block.attachmentId)
            if (source !== undefined && !sources.some((s) => s.id === source.id)) sources.push(source)
          }
          if (sources.length > 0) {
            const rightmost = sources.reduce((a, b) => (b.x >= a.x ? b : a))
            ...
            for (const source of sources) {
              const edgeResult = addEdge(next, { source: source.id, target: placeholder.node.id })
