...
function referenceImageIdsOf(event) {
    const data = event.data;
    const ids = [];
    ...
    for (const block of data.content) {
        ...
        if (image.type !== 'image')
            continue;
        if (typeof image.attachment?.attachmentId === 'string')
            ids.push(image.attachment.attachmentId);
    }
    return ids;
}
...
const currentTurnReferenceImages = new WeakMap();
...
export function apply(ctx) {
    ...
    ctx.on('session/event', (session, event) => {
        ...
        if (event.type === 'user/message') {
            ...
            if (ids.length > 0) {
                const set = currentTurnReferenceImages.get(session) ?? new Set();
                for (const id of ids)
                    set.add(id);
